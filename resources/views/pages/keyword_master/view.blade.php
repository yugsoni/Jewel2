...
@section('title', 'View All Keywords')
   ...
   <table class="table table-info table-hover">
    <thead>
      <tr class="table-primary">
        ...
        <th>Keywords Created At</th>
        <th>Edit</th>
        <th>Delete</th>
      </tr>
    </thead>
    <tbody>
      @foreach ($keyword_show as $keyword_shows)
        <tr>
          
          <td>{{ $id++ }}</td>
          <td>{{ $keyword_shows->Keyword }}</td>
          <td>{{ $keyword_shows->user_name }}</td>
          <td>{{ date('d M 20y', strtotime($keyword_shows->created_at)) }}</td>

          <td>
            <form action="{{ route('edit_k', $keyword_shows->id) }}">
              @csrf
              <button href="#" class="btn btn-success">Edit</button>
            </form>
        </td>
        <td>
<form action="{{ route('delete_k', $keyword_shows->id) }}" method="POST">
                        @csrf
