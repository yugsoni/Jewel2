    ...
    <tbody>
      @foreach ($category_show as $category_shows)
        <tr>
          ...
          <td>{{ $id++ }}</td>
          <td>{{ $category_shows->Category }}</td>
          <td>{{ $category_shows->user_name }}</td>
          <td>{{ date('d M 20y', strtotime($category_shows->created_at)) }}</td>

          <td>
            <form action="{{ route('edit_ca', $category_shows->id) }}">
              @csrf
              <button href="#" class="btn btn-success">Edit</button>
            </form>
        </td>
        <td>
<form action="{{ route('delete_ca', $category_shows->id) }}" method="POST">
                        @csrf
